    ...
    <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-50 pb-10">
      <div className="bg-white rounded-2xl p-6 mx-4 w-full max-w-sm shadow-xl">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className="flex-1 py-3 rounded-xl bg-red-400 text-white font-bold">삭제</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  const [recipeOrder, setRecipeOrder] = useState([]); // 사용자 지정 순서 (id 배열)
  ...
  // Firebase 준비 대기
  useEffect(() => {
    if (window._firebaseReady) { setReady(true); return; }
  ...
  useEffect(() => {
    if (!ready) return;
    const { collection, doc, onSnapshot } = window._firebase;
    const db = window._db;
    const unsubR = onSnapshot(collection(db, "recipes"), snap => {
      setRecipes(snap.docs.map(d => ({ ...d.data(), id: d.id })));
    });
    const unsubS = onSnapshot(doc(db, "settings", "main"), snap => {
      if (snap.exists()) {
        const d = snap.data();
  ...
  const saveSettings = async (s, c, ro) => {
    ...
    await setDoc(doc(window._db, "settings", "main"), data, { merge: true });
  };
  ...
  // 레시피 정렬: recipeOrder 기준, 없는 것은 updatedAt 내림차순으로 앞에
  const sortedRecipes = (() => {
    const orderMap = {};
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-5"
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-rose-400 mb-1"
          style={{ fontFamily: "Georgia, serif", letterSpacing: "-0.5px" }}>이안이의 이유식</h1>
        <p className="text-sm text-rose-300 font-medium">미연이의 이유식 레시피북 📖</p>
      </div>
  ...
  if (page === "list") return (
    <div className="min-h-screen" style={{ background: "#fff8f9" }}>
      <div className="sticky top-0 z-10 bg-white border-b border-rose-100 px-4 py-3 flex items-center gap-3">
        ...
        <h2 className="font-bold text-gray-700 text-lg flex-1">
          {sortMode ? "순서 변경 ✏️" : "레시피 목록"}
        </h2>
        {sortMode ? (
          <button onClick={()=>setSortMode(false)}
            className="bg-emerald-400 text-white rounded-full px-4 py-1.5 text-sm font-bold shadow-sm">완료</button>
        ) : (
          <div className="flex gap-2">
            <button onClick={()=>setSortMode(true)}
              className="border border-rose-200 text-rose-400 rounded-full px-3 py-1.5 text-sm font-bold">순서</button>
            <button onClick={()=>{ setForm(emptyRecipe(stages)); setEditId(null); setPage("add"); }}
              className="bg-rose-400 text-white rounded-full px-4 py-1.5 text-sm font-bold shadow-sm">+ 추가</button>
          </div>
        )}
      </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              ...
              {stages.map((s,i) => (
                <button key={s} onClick={()=>setSelectedStage(s)}
                  className={`flex-shrink-0 text-xs rounded-full px-3 py-1.5 font-semibold border transition-colors ${selectedStage===s?STAGE_C[i%STAGE_C.length]:"bg-white text-gray-500 border-gray-200"}`}>{s}</button>
              ))}
            </div>
          </div>
          <div className="px-4 pb-2">
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              <button onClick={()=>setSelectedCat(null)}
                className={`flex-shrink-0 text-xs rounded-full px-3 py-1.5 font-semibold border transition-colors ${!selectedCat?"bg-amber-400 text-white border-amber-400":"bg-white text-gray-500 border-gray-200"}`}>전체</button>
              {categories.map(cat => (
                <button key={cat} onClick={()=>setSelectedCat(cat)}
                  className={`flex-shrink-0 text-xs rounded-full px-3 py-1.5 font-semibold border transition-colors ${selectedCat===cat?"bg-amber-400 text-white border-amber-400":"bg-white text-gray-500 border-gray-200"}`}>{cat}</button>
              ))}
            </div>
          </div>
        </>
      ...
      {sortMode && (
        <p className="text-xs text-gray-400 text-center py-2">↑↓ 버튼으로 순서를 바꿀 수 있어요</p>
      )}
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  ...
                  {recipe.stage && (
                    <span className={`flex-shrink-0 text-xs px-2.5 py-1 rounded-full border font-semibold ${stageColor(recipe.stage)}`}>{recipe.stage}</span>
                  )}
                </div>
  ...
  if (page === "detail" && viewRecipe) return (
    <div className="min-h-screen" style={{ background: "#fff8f9" }}>
      <div className="sticky top-0 z-10 bg-white border-b border-rose-100 px-4 py-3 flex items-center gap-3">
        ...
        <button onClick={()=>{ setForm({...viewRecipe}); setEditId(viewRecipe.id); setPage("add"); }}
          className="text-sky-400 text-sm font-semibold mr-2">수정</button>
      ...
      <div className="px-4 pt-4 pb-10 space-y-5">
        <div className="flex flex-wrap gap-2 items-center">
          {viewRecipe.stage && (
            <span className={`text-xs px-3 py-1 rounded-full border font-semibold ${stageColor(viewRecipe.stage)}`}>📅 {viewRecipe.stage}</span>
          )}
  ...
  if (page === "add") return (
    <div className="min-h-screen" style={{ background: "#fff8f9" }}>
      <div className="sticky top-0 z-10 bg-white border-b border-rose-100 px-4 py-3 flex items-center gap-3">
        <button onClick={()=>{ setPage("list"); setForm(emptyRecipe(stages)); setEditId(null); }}
          className="text-rose-300 text-xl">←</button>
